...
import { COTRecord, matifToCOTRecord, MatifRecord } from "@/lib/cftc";
...
export function AllGlobalWheatTab() {
  const [data, setData] = useState<COTRecord[]>([]);
  const [chicagoData, setChicagoData] = useState<COTRecord[]>([]);
  const [kansasData, setKansasData] = useState<COTRecord[]>([]);
  const [minneapolisData, setMinneapolisData] = useState<COTRecord[]>([]);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        // Fetch all wheat contracts in parallel (US + Matif)
        const [chicagoRes, kansasRes, minneapolisRes, matifRes] = await Promise.all([
          fetch("/api/cot?contract=chicago-wheat"),
          fetch("/api/cot?contract=kansas-wheat"),
          fetch("/api/cot?contract=minneapolis-wheat"),
          fetch("/api/matif-cot?contract=matif-wheat&format=cot"),
        ]);

        const [chicagoJson, kansasJson, minneapolisJson, matifJson] = await Promise.all([
          chicagoRes.json(),
          kansasRes.json(),
          minneapolisRes.json(),
          matifRes.json(),
        ...
        if (!chicagoJson.success || !kansasJson.success || !minneapolisJson.success) {
          setError("Failed to fetch US wheat data");
          return;
        }

        // Store individual commodity data
        setChicagoData(chicagoJson.data);
        setKansasData(kansasJson.data);
        setMinneapolisData(minneapolisJson.data);

        // Matif data - convert if not already in COT format
        let matif: COTRecord[] = [];
        ...
        // Merge all the data
        const merged = mergeGlobalWheatData(
          chicagoJson.data,
          kansasJson.data,
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-zinc-500">
                CFTC COT - Chicago + Kansas + Minneapolis + Matif Wheat Combined
              </p>
            </div>
