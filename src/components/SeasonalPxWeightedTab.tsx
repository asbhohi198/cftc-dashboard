...
interface SeasonalPxWeightedTabProps {
  sector?: string;
}

export function SeasonalPxWeightedTab({ sector = "ags" }: SeasonalPxWeightedTabProps) {
  const [contracts, setContracts] = useState<PxWeightedRow[]>([]);
  ...
  const chartDataByContract = useMemo(() => {
    return contracts.map((contract) => ({
      id: contract.id,
      name: contract.name,
      notionalData: contract.historicalData.map((d) => ({
        date: d.date,
  ...
  const recordStatuses = useMemo(() => {
    ...
    chartDataByContract.forEach(({ name, notionalData }) => {
      const status = checkRecordStatus(notionalData);
      if (status.isMax || status.isMin) {
        records.push({ name: `${name} Notional`, isMax: status.isMax, isMin: status.isMin });
      }
    });
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-white">
                COT - Price Weighted: {sectorLabel} - Seasonal View
              </h2>
              <p className="text-xs text-zinc-500">
                Day-of-year overlay comparing multiple years (Notional Positions in $M)
              </p>
              {/* Record Highs/Lows */}
