...
function formatNotional(num: number): string {
...
interface COTPxWeightedTabProps {
  sector: string;
}

export function COTPxWeightedTab({ sector }: COTPxWeightedTabProps) {
  const [data, setData] = useState<PxWeightedRow[]>([]);
  ...
  const [expandedChart, setExpandedChart] = useState<"notional" | "change" | "pctMax" | "total" | string | null>(null);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/cot-px-weighted?sector=${sector}`);
        const json: APIResponse = await res.json();
        if (json.success) {
          setData(json.contracts);
          setSectorLabel(json.sectorLabel);
          setReportDate(json.reportDate);
          setTotalNotional(json.totalNotional);
  ...
  const calcAggregate = (rows: PxWeightedRow[]) => {
    if (rows.length === 0) return null;

    const dateMap = new Map<string, { notionalValue: number; count: number }>();
    for (const row of rows) {
      for (const h of row.historicalData) {
        const existing = dateMap.get(h.date);
        if (existing) {
          existing.notionalValue += h.notionalValue;
          existing.count++;
        } else {
          dateMap.set(h.date, { notionalValue: h.notionalValue, count: 1 });
        }
      }
    }
    ...
    const historicalData = Array.from(dateMap.entries())
      ...
      .map(([date, v]) => ({ date, notionalValue: v.notionalValue }))
    ...
    const currentNotional = historicalData.length > 0 ? historicalData[historicalData.length - 1].notionalValue : 0;
    const pctMax = recordMax > 0 ? (currentNotional / recordMax) * 100 : 0;
  ...
  const renderTableRow = (row: PxWeightedRow) => (
    <tr
      key={row.id}
    ...
    >
      ...
      <td className={`py-1.5 px-1 text-center font-mono font-bold ${row.notionalValue >= 0 ? "text-green-400" : "text-red-400"}`}>
        {row.notionalValue >= 0 ? "+" : ""}{formatNotional(row.notionalValue)}
      </td>
      <td className={`py-1.5 px-1 text-center font-mono ${row.change >= 0 ? "text-green-400" : "text-red-400"}`}>
        {row.change >= 0 ? "+" : ""}{formatNotional(row.change)}
      </td>
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-white">
              COT - Price Weighted: {sectorLabel}
            </h2>
            <p className="text-xs text-zinc-500 mt-1">
              MM Net × Price = Notional Position (in $M)
            </p>
          </div>
          <div className="flex items-center gap-6">
            <div className="text-right">
              <p className="text-xs text-zinc-500">Total Notional</p>
              <p className={`text-lg font-bold ${totalNotional >= 0 ? "text-green-400" : "text-red-400"}`}>
                {totalNotional >= 0 ? "+" : ""}{formatNotional(totalNotional)}
              </p>
            </div>
            {reportDate && (
              <div className="text-right">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-4">
          ...
          <table className="w-full text-xs table-fixed">
            <thead>
              <tr className="bg-zinc-800/50 border-b border-zinc-700">
                ...
                <th className="text-center py-2 px-1 text-zinc-400 font-medium">Notional</th>
        ...
        <div className="xl:col-span-3 grid grid-cols-2 gap-3">
          {/* Notional Positions Bar Chart */}
          <div
            className="bg-zinc-900 border border-zinc-800 rounded-lg p-2 cursor-pointer hover:border-zinc-600 transition-colors"
            onClick={() => setExpandedChart("notional")}
          >
            <h3 className="text-sm font-semibold text-white mb-2">MM Notional Position ($M)</h3>
            <div className="h-[240px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={notionalChartData} margin={{ top: 12, right: 5, left: -25, bottom: 30 }}>
                  ...
                  <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                    <LabelList
                      ...
                      fill="#ffffff"
                      fontSize={9}
                      formatter={(value: number) => formatNotional(value)}
                    />
                    {notionalChartData.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
          ...
          >
            <h3 className="text-sm font-semibold text-white mb-2">Notional Chg WoW ($M)</h3>
            <div className="h-[240px]">
              <ResponsiveContainer width="100%" height="100%">
          ...
          >
            <h3 className="text-sm font-semibold text-white mb-2">Notional % of Record Max</h3>
            <div className="h-[240px]">
              <ResponsiveContainer width="100%" height="100%">
          ...
          {/* Total Notional Historical Chart */}
          {totalAgg && (
            <div
              className="bg-zinc-900 border border-zinc-800 rounded-lg p-2 cursor-pointer hover:border-zinc-600 transition-colors"
              onClick={() => setExpandedChart("total")}
            >
              <h3 className="text-sm font-semibold text-white mb-2">Total Notional (Historical)</h3>
              <div className="h-[240px]">
                <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-white">Individual Commodity Notional Positions</h3>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-white">
                {expandedChart === "notional" && "MM Notional Position ($M)"}
                {expandedChart === "change" && "Notional Chg WoW ($M)"}
                {expandedChart === "pctMax" && "Notional % Max"}
                {expandedChart === "total" && "Total Notional Position (Historical)"}
                {expandedChart?.startsWith("individual-") && `${data.find(d => d.id === expandedChart.replace("individual-", ""))?.name || ""} Notional ($M)`}
              </h3>
              <button
                onClick={() => setExpandedChart(null)}
            ...
            <div className="h-[665px]">
              <ResponsiveContainer width="100%" height="100%">
                {(expandedChart === "notional" || expandedChart === "change" || expandedChart === "pctMax") ? (
                  <BarChart
                    data={
                      expandedChart === "notional" ? notionalChartData :
                      ...
                      {(expandedChart === "notional" ? notionalChartData :
                        ...
                        <Cell
                          ...
                          fill={expandedChart === "pctMax" ? "#3b82f6" :
                            expandedChart === "notional" ? (entry.value >= 0 ? "#22c55e" : "#ef4444") :
