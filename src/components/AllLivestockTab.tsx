...
function mergeLivestockData(datasets: COTRecord[][]): COTRecord[] {
...
export function AllLivestockTab() {
  const [data, setData] = useState<COTRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const contractName = "All Livestock";
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        // Fetch all livestock contracts in parallel
        const [liveCattleRes, feederCattleRes, leanHogsRes] = await Promise.all([
          fetch("/api/cot?contract=live-cattle"),
          fetch("/api/cot?contract=feeder-cattle"),
          fetch("/api/cot?contract=lean-hogs"),
        ]);

        const [liveCattleJson, feederCattleJson, leanHogsJson] = await Promise.all([
          liveCattleRes.json(),
          feederCattleRes.json(),
          leanHogsRes.json(),
        ]);

        if (!liveCattleJson.success || !feederCattleJson.success || !leanHogsJson.success) {
          setError("Failed to fetch livestock data");
          return;
        }

        // Merge the data
        const merged = mergeLivestockData([
          liveCattleJson.data,
          feederCattleJson.data,
          leanHogsJson.data,
        ]);
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-zinc-500">
                CFTC COT - Live Cattle + Feeder Cattle + Lean Hogs Combined
              </p>
            </div>
