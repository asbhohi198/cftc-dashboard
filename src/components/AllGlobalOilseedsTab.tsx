...
function mergeGlobalOilseedsData(
  soybeans: COTRecord[],
  soymeal: COTRecord[],
  soyoil: COTRecord[],
  canola: COTRecord[],
  matifRapeseed: COTRecord[]
): COTRecord[] {
  // Collect all unique dates from all sources
  const allDates = new Set<string>();
  for (const rec of soybeans) allDates.add(rec.date);
  for (const rec of soymeal) allDates.add(rec.date);
  for (const rec of soyoil) allDates.add(rec.date);
  for (const rec of canola) allDates.add(rec.date);
  for (const rec of matifRapeseed) allDates.add(rec.date);
  ...
  for (const date of sortedDates) {
    const soybeansRec = getRecordAtDate(soybeans, date) || zeroRecord(date);
    const soymealRec = getRecordAtDate(soymeal, date) || zeroRecord(date);
    const soyoilRec = getRecordAtDate(soyoil, date) || zeroRecord(date);
    const canolaRec = getRecordAtDate(canola, date) || zeroRecord(date);
    const matifRec = getRecordAtDate(matifRapeseed, date) || zeroRecord(date);

    // Sum all five with the correct date
    let merged = { ...soybeansRec, date };
    merged = sumRecords(merged, { ...soymealRec, date });
    merged = sumRecords(merged, { ...soyoilRec, date });
    merged = sumRecords(merged, { ...canolaRec, date });
...
export function AllGlobalOilseedsTab() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        // Fetch all oilseed contracts in parallel (US + Canola + Matif Rapeseed)
        const [soybeansRes, soymealRes, soyoilRes, canolaRes, matifRes] = await Promise.all([
          fetch("/api/cot?contract=soybeans"),
          fetch("/api/cot?contract=soymeal"),
          fetch("/api/cot?contract=soyoil"),
          fetch("/api/cot?contract=canola"),
          fetch("/api/matif-cot?contract=matif-rapeseed&format=cot"),
        ]);

        const [soybeansJson, soymealJson, soyoilJson, canolaJson, matifJson] = await Promise.all([
          soybeansRes.json(),
          soymealRes.json(),
          soyoilRes.json(),
          canolaRes.json(),
        ...
        if (!soybeansJson.success || !soymealJson.success || !soyoilJson.success) {
          setError("Failed to fetch US oilseeds data");
          return;
        }
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-zinc-500">
                CFTC COT - Soybeans + Soymeal + Soyoil + Canola + Matif Rapeseed Combined
              </p>
            </div>
